...
export function EvidencePlan({ actions, sources }: { actions: EvidenceAction[]; sources?: SourceAnalysis }) {
  ...
  return <section aria-label="Доказательный план действий">
    ...
    <aside className="analytics-card"><h3>Что мы знаем о выборе источников</h3>
      <p>Ссылки показывают, какие ресурсы указаны в сохранённых ответах. Они не раскрывают закрытый алгоритм YandexGPT API или генеративного поиска Яндекса и не доказывают, что публикация вызвала рекомендацию.</p>
      <p>Для проверки гипотез нужны повторные наблюдения по тем же вопросам, региону и каналу, история публикаций и сравнение с вопросами без изменений. Ответы API-моделей учитываются отдельно от пользовательской Алисы, которая здесь не измеряется.</p>
