...
type ReportShape = {
  ...
  publication_learning?: { status: string; explanation: string; experiments: Array<{ id: number; publication_id: number; baseline_research_id: number; followup_research_id: number; evidence_grade: string; causality_status: string; metric_deltas: Record<string, number>; sample_size: number }>; influence_estimates: Array<{ id: number; resource_domain: string; channel: string; content_type: string; metric: string; provider: string; model: string; sample_size: number; expected_delta: number; confidence_min: number; confidence_max: number; confidence_score: number; evidence_grade: string }> };
};
  ...
  return (
    <main className="page report-page">
      ...
      <section className="panel research-lab-section"><span className="section-label">ОБУЧЕНИЕ НА ПУБЛИКАЦИЯХ</span><h2>Что влияет на рекомендации ИИ</h2><p>{report.publication_learning?.explanation ?? "Для обучения нужны публикации и повторные исследования с одинаковой матрицей."}</p>
        <div className="publication-form"><h3>Зарегистрировать новый материал</h3><p>Сохраните вышедшую публикацию, затем повторите те же вопросы тем же моделям.</p><input aria-label="Название публикации" placeholder="Название материала" value={publicationTitle} onChange={(event) => setPublicationTitle(event.target.value)} /><input aria-label="URL публикации" placeholder="https://..." value={publicationUrl} onChange={(event) => setPublicationUrl(event.target.value)} /><button type="button" disabled={!publicationTitle.trim() || !publicationUrl.trim()} onClick={async () => { try { const entityId = String((report.research as Record<string, unknown> | undefined)?.entity_id ?? ""); if (!entityId) throw new Error("У исследования отсутствует entity_id"); const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(publicationUrl.trim())); const contentHash = Array.from(new Uint8Array(digest)).map((byte) => byte.toString(16).padStart(2, "0")).join(""); await api.createResearchPublication({ entity_id: entityId, research_id: result.research.id, url: publicationUrl.trim(), content_hash: contentHash, title: publicationTitle.trim(), channel: "EARNED", content_type: "ARTICLE", target_queries: report.query_catalog?.map((item) => item.text) ?? [], published_at: new Date().toISOString() }); setPublicationStatus("Публикация сохранена. Теперь повторите исследование с теми же вопросами."); setPublicationTitle(""); setPublicationUrl(""); } catch (error) { setPublicationStatus(error instanceof Error ? error.message : "Не удалось сохранить публикацию"); } }}>Сохранить публикацию</button>{publicationStatus ? <p>{publicationStatus}</p> : null}</div>
        {report.publication_learning?.influence_estimates.length ? report.publication_learning.influence_estimates.slice(0, 12).map((item) => <article className="influence-estimate" key={item.id}><h3>{item.resource_domain} · {metricNames[item.metric] ?? item.metric}</h3><p>{item.provider === "ALL" ? "Все модели" : `${item.provider}/${item.model}`} · {item.channel} · {item.content_type}</p><div className="action-meta"><div><span>Наблюдаемый эффект</span><b>{item.expected_delta >= 0 ? "+" : ""}{item.expected_delta.toFixed(1)}</b></div><div><span>Диапазон</span><b>{item.confidence_min.toFixed(1)}…{item.confidence_max.toFixed(1)}</b></div><div><span>Наблюдений</span><b>{item.sample_size}</b></div><div><span>Уверенность</span><b>{Math.round(item.confidence_score * 100)}%</b></div></div><small>{item.evidence_grade}; результат не является гарантией причинного влияния.</small></article>) : <p className="empty-state">Пока недостаточно сопоставимых исследований. После публикации повторите тот же набор запросов и моделей.</p>}
      </section>
