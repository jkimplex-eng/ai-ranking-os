...
function GeoOpportunitiesScreen() {
  ...
  const [wordstatForm, setWordstatForm] = useState({ category: "онлайн-образование", region: "213", device: "all" as "all" | "desktop" | "phone" | "tablet" });
  ...
  const discoverWordstat = async () => {
    if (!selectedBrand || !wordstatForm.category.trim()) { setError("Выберите исследование бренда и укажите категорию"); return; }
    setBusy(true); setError(""); setOperationResult("");
    try {
      const snapshot = await api.discoverWordstat({ brand: selectedBrand, category: wordstatForm.category.trim(), region_ids: wordstatForm.region ? [Number(wordstatForm.region)] : [], device: wordstatForm.device, limit: 20 });
      setWordstatSnapshot(snapshot);
      setWordstatAnalytics(await api.wordstatAnalytics(selectedBrand));
      setOperationResult(`Wordstat нашёл ${snapshot.queries.length} запросов. Они будут автоматически добавлены в следующее исследование бренда ${selectedBrand}.`);
