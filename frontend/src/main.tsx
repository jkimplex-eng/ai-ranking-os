...
import {
  ...
  type WordstatAnalytics,
  type WordstatConnection,
  type WordstatSnapshot,
  type WizardPayload,
  type WizardReview,
} from "./api";
import {
...
type ReportShape = {
  ...
  explainability?: {
    ...
    evidence_methodology_version?: string;
    historical_score_warning?: string | null;
    ...
    option_measurement?: { status: string; option_responses: number; measured_responses: number; rate_percent: number | null; evidence_response_ids: number[]; verdict_version: string | null; limitation: string };
    ...
    sample_scope?: { query_count: number; response_count: number; successful_response_count: number; failed_response_count: number; unbranded_response_count?: number; successful_unbranded_response_count?: number; excluded_branded_response_ids?: number[]; provider_model_count: number; languages: string[]; regions: string[]; limitation: string };
  };
...
function Shell({
  ...
  const expertNavSource = [
    ...
    ["⌘", "Карта бренда", "graph"], ["◈", "Где публиковаться", "geo"],
...
function SettingsScreen({ user }: { user: string }) {
  ...
  const [wordstat, setWordstat] = useState<WordstatConnection | null>(null);
  const [wordstatForm, setWordstatForm] = useState({ folder_id: "", auth_type: "API_KEY" as "API_KEY" | "IAM_TOKEN", credential: "" });
  ...
  useEffect(() => { api.wordstatStatus().then((status) => { setWordstat(status); if (status.folder_id) setWordstatForm((current) => ({ ...current, folder_id: status.folder_id || "", auth_type: status.auth_type || "API_KEY" })); }).catch(() => setWordstat(null)); }, []);
  ...
  return <main className="analytics-page settings-page"><header className="analytics-hero"><div><span className="eyebrow">PREFERENCES</span><h1>Настройки</h1><p>Единый центр персональных и системных настроек.</p></div><button className="primary-action" onClick={() => api.updateWorkspace(settings).then(() => setSaved(true))}>{saved ? "Сохранено ✓" : "Сохранить"}</button></header>
    ...
    <div className="settings-layout"><nav className="settings-nav">{tabs.map(([key, label]) => <button className={tab === key ? "active" : ""} onClick={() => setTab(key)} key={key}>{label}</button>)}</nav><section className="analytics-card settings-panel">
      ...
      {tab === "integrations" && <><h2>Yandex Wordstat — спрос категории</h2><p>Основной источник частотных запросов. Подтверждать права на сайт не нужно: Wordstat находит популярные формулировки категории, которые затем можно проверить через YandexGPT.</p><div className="setting-row"><span>Доступ Search API</span><Badge tone={wordstat?.connected ? "success" : "warning"}>{wordstat?.connected ? "ПОДКЛЮЧЁН" : "НЕ ПОДКЛЮЧЁН"}</Badge></div>{wordstat?.connected ? <><div className="setting-row"><span>Каталог Yandex Cloud</span><b>{wordstat.folder_id}</b></div><div className="button-row"><button className="secondary" disabled={wordstatBusy} onClick={() => { setWordstatBusy(true); api.disconnectWordstat().then(() => { setWordstat({ connected: false, status: "NOT_CONFIGURED" }); setWordstatForm((current) => ({ ...current, credential: "" })); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Не удалось отключить Wordstat")).finally(() => setWordstatBusy(false)); }}>Отключить Wordstat</button></div></> : <><label>Идентификатор каталога<input value={wordstatForm.folder_id} onChange={(event) => setWordstatForm({ ...wordstatForm, folder_id: event.target.value })} placeholder="b1g…" /></label><label>Способ авторизации<select value={wordstatForm.auth_type} onChange={(event) => setWordstatForm({ ...wordstatForm, auth_type: event.target.value as "API_KEY" | "IAM_TOKEN" })}><option value="API_KEY">API-ключ</option><option value="IAM_TOKEN">IAM-токен</option></select></label><label>{wordstatForm.auth_type === "API_KEY" ? "API-ключ" : "IAM-токен"}<input type="password" value={wordstatForm.credential} onChange={(event) => setWordstatForm({ ...wordstatForm, credential: event.target.value })} autoComplete="off" /></label><button className="primary-action" disabled={wordstatBusy || !wordstatForm.folder_id.trim() || !wordstatForm.credential.trim()} onClick={() => { setWordstatBusy(true); setError(""); api.connectWordstat(wordstatForm).then((status) => { setWordstat(status); setWordstatForm((current) => ({ ...current, credential: "" })); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Не удалось подключить Wordstat")).finally(() => setWordstatBusy(false)); }}>{wordstatBusy ? "Проверяем…" : "Проверить и подключить"}</button><p className="empty-state">Нужен сервисный аккаунт с ролью search-api.webSearch.user и API-ключ с областью yc.search-api.execute. Секрет хранится зашифрованно и после сохранения не показывается.</p></>}<hr /><h2>Дополнительно: Яндекс Вебмастер для владельца сайта</h2><p>Не требуется для анализа категории и конкурентов. Подключайте только если вы владелец подтверждённого сайта и хотите добавить позиции, показы и страницы.</p>
...
function GraphScreen() {
  ...
  if (!researches.length) return <main className="analytics-page graph-page"><header className="analytics-hero"><div><span className="eyebrow">КАРТА БРЕНДА</span><h1>Что система узнала о бренде</h1><p>Здесь появятся сущности, источники и связи, извлечённые из завершённого исследования.</p></div></header><div className="analytics-card empty-state"><h2>Карты пока нет</h2><p>Завершите первое исследование. SIGNAL извлечёт из сохранённых ответов бренды, сайты, продукты и подтверждённые связи между ними.</p></div></main>;
  ...
  if (error) return <main className="analytics-page graph-page"><header className="analytics-hero"><div><span className="eyebrow">{selectedBrand || "ВЫБРАННЫЙ БРЕНД"}</span><h1>Карта бренда пока недоступна</h1><p>Для выбранного исследования сущности и связи ещё не были подготовлены.</p></div>{selector}</header><div className="analytics-card empty-state" role="alert"><h2>Что можно сделать</h2><p>{error}</p><p>Выберите другое завершённое исследование или запустите новое. Карта появится после обработки ответов.</p></div></main>;
  ...
  const typeLabel = (type: string) => ({ PERSON: "Человек", ORGANIZATION: "Организация", PRODUCT: "Продукт", SERVICE: "Сервис", WEBSITE: "Сайт", DOMAIN: "Сайт", ARTICLE: "Публикация", BRAND: "Бренд", TOPIC: "Тема" }[type.toUpperCase()] ?? "Другая сущность");
  const connectionCount = (nodeId: number) => graph.edges.filter((edge) => edge.source_node_id === nodeId || edge.target_node_id === nodeId).length;
  const ordered = [...visible].sort((left, right) => connectionCount(right.id) - connectionCount(left.id) || right.confidence - left.confidence || left.name.localeCompare(right.name, "ru"));
  const selectedNode = graph.nodes.find((node) => node.id === selected) ?? ordered[0];
  const connected = selectedNode ? graph.edges.filter((edge) => edge.source_node_id === selectedNode.id || edge.target_node_id === selectedNode.id) : [];
  const otherNode = (edge: GraphEdge) => graph.nodes.find((node) => node.id === (edge.source_node_id === selectedNode?.id ? edge.target_node_id : edge.source_node_id));
  const highConfidence = graph.nodes.filter((node) => node.confidence >= 0.7).length;
  return <main className="analytics-page graph-page"><header className="analytics-hero"><div><span className="eyebrow">{selectedBrand} · ИССЛЕДОВАНИЕ #{researchId}</span><h1>Карта бренда и источников</h1><p>Показывает, что удалось извлечь из сохранённых ответов: бренды, сайты, сервисы и связи между ними. Это справочная карта исследования, а не список конкурентов или готовый план публикаций.</p></div>{selector}</header>
    <section className="analytics-card graph-purpose"><div><span className="eyebrow">ЧТО ЭТО ДАЁТ</span><h2>Проверьте, кого и что система увидела рядом с брендом</h2><p>Выберите сущность, чтобы посмотреть найденные связи. Для плана действий используйте «Что делать», а для площадок — «Где публиковаться».</p></div><div className="graph-purpose-links"><span>1. Найти сущность</span><span>2. Проверить связь</span><span>3. Перейти к действию</span></div></section>
    <section className="graph-summary" aria-label="Сводка карты"><article><strong>{graph.node_count}</strong><span>объектов найдено</span><small>Бренды, сайты, сервисы и другие упоминания</small></article><article><strong>{graph.edge_count}</strong><span>связей найдено</span><small>Только связи, зафиксированные в исследовании</small></article><article><strong>{highConfidence}</strong><span>с высокой уверенностью</span><small>Уверенность извлечения от 70%</small></article></section>
    <section className="analytics-card graph-toolbar"><div><label>Найти объект<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Например, название сервиса или сайта" /></label><label>Тип<select value={nodeType} onChange={(event) => setNodeType(event.target.value)}><option value="ALL">Все типы</option>{types.map((type) => <option value={type} key={type}>{typeLabel(type)}</option>)}</select></label></div><p>{edges.length ? "Выберите строку, чтобы увидеть её связи." : "Связи в этой выборке не обнаружены: это не означает, что их не существует вне исследования."}</p></section>
    {!ordered.length ? <div className="analytics-card empty-state">По выбранным условиям объекты не найдены.</div> : <section className="graph-map-layout"><article className="analytics-card graph-entity-list"><header><div><span className="eyebrow">ОБНАРУЖЕНО В ОТВЕТАХ</span><h2>{ordered.length} объектов</h2></div><small>Сортировка: сначала объекты с большим числом связей</small></header><div className="graph-entity-rows">{ordered.map((node) => <button type="button" className={selectedNode?.id === node.id ? "active" : ""} onClick={() => setSelected(node.id)} key={node.id}><span className="graph-entity-icon">{typeLabel(node.node_type).slice(0, 1)}</span><span><b>{node.name}</b><small>{typeLabel(node.node_type)}{node.aliases.length ? ` · также: ${node.aliases.slice(0, 2).join(", ")}` : ""}</small></span><span className="graph-entity-metric"><b>{connectionCount(node.id)}</b><small>{connectionCount(node.id) === 1 ? "связь" : "связей"}</small></span></button>)}</div></article><aside className="analytics-card graph-detail">{selectedNode ? <><span className="eyebrow">{typeLabel(selectedNode.node_type)}</span><h2>{selectedNode.name}</h2><p className="graph-detail-confidence">Уверенность извлечения: <b>{(selectedNode.confidence * 100).toFixed(0)}%</b></p><p>{selectedNode.aliases.length ? `Также встречается как: ${selectedNode.aliases.join(", ")}.` : "Другие варианты названия в этом исследовании не зафиксированы."}</p><h3>Найденные связи</h3>{connected.length ? connected.map((edge) => { const related = otherNode(edge); return <article className="graph-connection" key={edge.id}><span>{edge.edge_type.replaceAll("_", " ")}</span><b>{related?.name ?? "Объект не найден"}</b><small>Уверенность связи {(edge.confidence * 100).toFixed(0)}%{Object.keys(edge.properties ?? {}).length ? " · есть сохранённый контекст" : ""}</small></article>; }) : <div className="empty-state"><b>Подтверждённых связей нет</b><p>Объект встретился в ответах, но система не нашла надёжной связи с другим объектом. Это не рекомендация и не конкурентный вывод.</p></div>}<details className="graph-technical"><summary>Технические данные объекта</summary><p>Тип в данных: {selectedNode.node_type}</p><p>ID: {selectedNode.external_id}</p></details></> : null}</aside></section>}
  </main>;
}
...
function RecordsScreen({ kind, onNewResearch }: { kind: RecordsKind; onNewResearch: () => void }) {
  ...
  const titles: Record<RecordsKind, [string, string]> = {
    ...
    graph: ["Карта бренда", "Объекты и связи выбранного исследования"],
...
function CompetitorsScreen() {
  ...
  const startSuggestedCompetitorMonitoring = async (candidate: Awaited<ReturnType<typeof api.competitorSuggestions>>[number]) => {
    if (!projectId) return;
    const requestNumber = ++dashboardRequest.current;
    setSaving(true); setLoading(true); setError("");
    try {
      const evidence = candidate.evidence.map((item) => ({ research_id: item.research_id, response_id: item.response_id, query: item.query, provider: item.provider, model: item.model, urls: item.urls }));
      await api.createProjectCompetitor(projectId, {
        name: candidate.name,
        domains: [],
        brands: [candidate.name],
        notes: `Автоматически добавлен из ответов исследования. ${candidate.limitation}\nДоказательства: ${JSON.stringify(evidence)}`,
      });
      const current = dashboard ?? await api.competitorDashboard(projectId);
      if (!current.monitoring_enabled) await api.setCompetitorDailyMonitoring(projectId, true);
      const refreshed = await api.refreshCompetitorDashboard(projectId);
      if (requestNumber === dashboardRequest.current) setDashboard(refreshed);
    } catch (reason) {
      if (requestNumber === dashboardRequest.current) setError(reason instanceof Error ? reason.message : "Не удалось начать мониторинг конкурента");
      throw reason;
    } finally {
      if (requestNumber === dashboardRequest.current) setLoading(false);
      setSaving(false);
  ...
  return <main className="analytics-page competitor-page">
    ...
    {!projects.length ? <section className="analytics-card competitor-first-project"><div><span className="eyebrow">ПЕРВЫЙ ШАГ</span><h2>Создайте проект для вашего бренда</h2><p>Проект объединяет ваш бренд, исследования и конкурентов. Например: «Skinjestique».</p></div><form onSubmit={createProject}><label htmlFor="competitor-project-name">Название проекта</label><div><input id="competitor-project-name" value={projectName} onChange={(event) => setProjectName(event.target.value)} placeholder="Название вашего бренда" required autoFocus /><button className="primary-action" disabled={creatingProject}>{creatingProject ? "Создаём…" : "Создать и продолжить"}</button></div></form></section> : <>
      <section className="competitor-controls">
        {projectId && <CompetitorSuggestions api={api} projectId={projectId} onStart={startSuggestedCompetitorMonitoring} />}
...
function ExpertGuideScreen({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  const tools = [
    ...
    { icon: "⌘", title: "Проверить карту бренда", text: "Посмотрите реальные бренды, сайты и связи, найденные в ответах моделей.", action: "Открыть карту", target: "graph" as Screen },
...
type PublicationTaskDraft = {
  ...
  editorial_status: "NOT_CHECKED" | "CONFIRMED" | "NOT_AVAILABLE";
  editorial_rules_url: string;
...
function GeoOpportunitiesScreen() {
    ...
    const recover = async <T,>(label: string, request: Promise<T>, fallback: T): Promise<T> => {
      try { return await request; }
        ...
        if (!(label === "снимок Вебмастера" && reason instanceof ApiError && reason.status === 404)) {
          ...
          errors[label] = reason instanceof Error ? reason.message : "Ошибка запроса";
        }
        return fallback;
      }
    };
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(ACTIVE_RESEARCH_KEY, String(selectedResearchId));
    const timer = window.setTimeout(() => {
      setResearchEvidence(undefined); setEvidenceError("");
      ...
      api.finalReport(selectedResearchId).then((report) => {
        ...
        // Completed Yandex measurements create their observed candidates on
        // the server. Reload the small registry here so the publication plan
        // is visible immediately rather than requiring a page refresh.
        api.geoPlatforms().then((items) => { if (!cancelled) setPlatforms(items); }).catch(() => undefined);
      }).catch((reason) => { if (!cancelled) setEvidenceError(reason instanceof Error ? reason.message : "Ошибка загрузки отчёта"); });
    }, 0);
  ...
  const auditCrawlScope = siteAudit && typeof siteAudit.evidence.crawl_scope === "object"
    ? siteAudit.evidence.crawl_scope as { pages_discovered?: number; pages_scanned?: number; max_pages?: number; strategy?: string }
    : undefined;
  const auditRoadmap = siteAudit && typeof siteAudit.evidence.roadmap === "object"
    ? siteAudit.evidence.roadmap as { current?: number; after_priority_actions?: number; after_all_detected_actions?: number; condition?: string }
    : undefined;
  const auditKnowledgeGraph = siteAudit && typeof siteAudit.evidence.knowledge_graph === "object"
    ? siteAudit.evidence.knowledge_graph as {
      nodes?: Array<{ id: string; type: string; label: string }>;
      edges?: Array<{ from: string; to: string; relation: string }>;
    }
    : undefined;
  const auditPageIssues = siteAudit && Array.isArray(siteAudit.evidence.page_issues)
    ? siteAudit.evidence.page_issues as Array<{ url?: string; signals?: Array<{ signal?: string; action?: string }> }>
    : [];
  ...
  return (
    <main className="analytics-page geo-page">
      ...
      {Object.keys(loadErrors).length > 0 && <Button disabled={loading} onClick={() => void load()}>Повторить загрузку данных</Button>}
      ...
      <section className="analytics-card geo-site-audit">
        ...
        {siteAudit ? <div className="geo-audit-result"><header><div><strong>{siteAudit.score.toFixed(1)}</strong><span>из 100 · {siteAudit.grade}</span></div><Badge tone={siteAudit.score >= 70 ? "success" : siteAudit.score >= 45 ? "warning" : "danger"}>v{siteAudit.algorithm_version}</Badge></header><p className="method-note"><b>Как получена оценка:</b> сервис заново загрузил {siteAudit.final_url}, robots.txt и sitemap. За каждый реально обнаруженный признак начислены указанные ниже баллы; итог — простая сумма без скрытой AI-оценки. Это готовность сайта быть понятным источником, а не вероятность рекомендации бренда.</p>{auditCrawlScope ? <p className="method-note"><b>Охват аудита:</b> найдено URL: {auditCrawlScope.pages_discovered ?? "—"}; прочитано HTML-страниц: {auditCrawlScope.pages_scanned ?? "—"}; лимит: {auditCrawlScope.max_pages ?? "—"}. {auditCrawlScope.strategy ?? ""}</p> : null}{auditRoadmap ? <div className="geo-audit-categories"><div><span>Сейчас</span><b>{auditRoadmap.current ?? siteAudit.score}</b></div><div><span>После 3 главных проверок</span><b>{auditRoadmap.after_priority_actions ?? "—"}</b></div><div><span>После всех найденных проверок</span><b>{auditRoadmap.after_all_detected_actions ?? "—"}</b></div></div> : null}{auditRoadmap?.condition ? <p className="method-note">{auditRoadmap.condition}</p> : null}<div className="geo-audit-categories">{Object.entries(siteAudit.category_scores).map(([name, value]) => <div key={name}><span>{name}</span><b>{value.toFixed(0)}</b></div>)}</div><h3>Главные действия</h3>{siteAudit.opportunities.slice(0, 5).map((item) => <article className="geo-audit-action" key={`${item.problem}:${item.affected_metric}`}><Badge tone={item.priority === "P0" ? "danger" : "warning"}>{item.priority}</Badge><div><b>{item.problem}</b><p>{item.action}</p><small>{item.expected_effect} · Проверка: {item.verification}</small>{item.checked_url ? <small>Проверенная страница: <a href={item.checked_url} target="_blank" rel="noreferrer">{item.checked_url}</a></small> : null}</div></article>)}<details><summary>Проблемы по найденным страницам · {auditPageIssues.length}</summary>{auditPageIssues.length ? auditPageIssues.map((page, index) => <article className="geo-audit-action" key={`${page.url}:${index}`}><div><b>{page.url ? <a href={page.url} target="_blank" rel="noreferrer">{page.url}</a> : "URL не записан"}</b>{page.signals?.map((signal) => <p key={signal.signal}><b>{signal.signal}:</b> {signal.action}</p>)}</div></article>) : <p>На просмотренных HTML-страницах таких проблем не обнаружено.</p>}</details>{auditKnowledgeGraph ? <details><summary>Knowledge Graph сайта: {auditKnowledgeGraph.nodes?.length ?? 0} узлов · {auditKnowledgeGraph.edges?.length ?? 0} связей</summary><p className="method-note">Граф строится только из реально прочитанных страниц, JSON-LD и ссылок. Отсутствующая сущность не добавляется в граф как факт.</p>{auditKnowledgeGraph.nodes?.slice(0, 30).map((node) => <div className="geo-audit-check" key={node.id}><span>●</span><div><b>{node.label}</b><small>{node.type}</small></div></div>)}{auditKnowledgeGraph.edges?.slice(0, 40).map((edge) => <div className="geo-audit-check" key={`${edge.from}:${edge.relation}:${edge.to}`}><span>→</span><div><b>{edge.relation}</b><small>{edge.from} → {edge.to}</small></div></div>)}</details> : null}<details><summary>Показать все доказательства расчёта</summary>{siteAudit.checks.map((check) => <div className="geo-audit-check" key={check.code}><span>{check.passed ? "✓" : "×"}</span><div><b>{check.title}</b><small>{check.evidence}</small>{check.checked_url ? <small>Проверенная страница: <a href={check.checked_url} target="_blank" rel="noreferrer">{check.checked_url}</a></small> : null}{check.recommendation ? <p>{check.recommendation}</p> : null}</div><strong>{check.points}/{check.max_points}</strong></div>)}</details><p className="method-note">{siteAudit.limitation}</p></div> : <div className="geo-empty"><strong>Аудит ещё не выполнялся</strong><p>Введите официальный сайт. Система не подставит оценку без фактического чтения страницы, robots.txt и sitemap.</p></div>}
      </section>
      ...
      <section className="analytics-card geo-site-audit yandex-intelligence">
        ...
        {wordstatSnapshot ? <div className="geo-audit-result"><header><div><strong>{wordstatSnapshot.queries.length}</strong><span>частотных запросов · {wordstatSnapshot.region_ids.length ? `регион ${wordstatSnapshot.region_ids.join(", ")}` : "вся Россия"}</span></div><Badge tone="success">WORDSTAT</Badge></header><div className="geo-audit-categories"><div><span>Суммарная частотность</span><b>{wordstatSnapshot.queries.reduce((sum, item) => sum + item.frequency, 0).toLocaleString("ru-RU")}</b></div><div><span>Проверено в YandexGPT</span><b>{wordstatAnalytics?.checked_query_count ?? 0}/{wordstatSnapshot.queries.length}</b></div><div><span>Взвешенная видимость</span><b>{wordstatAnalytics?.weighted_visibility == null ? "Нет данных" : `${wordstatAnalytics.weighted_visibility.toFixed(1)}%`}</b></div></div><h3>Запрос → частотность → результат YandexGPT</h3>{wordstatSnapshot.queries.slice(0, 30).map((query) => { const measured = wordstatAnalytics?.items.find((item) => item.query.toLocaleLowerCase() === query.query.toLocaleLowerCase()); return <div className="geo-audit-check" key={query.query}><span>{measured?.evidence_status === "MEASURED" ? (measured.recommendation_count ? "✓" : "×") : "?"}</span><div><b>{query.query}</b><small>{measured?.evidence_status === "MEASURED" ? `${measured.response_count} ответов · упоминание ${measured.mention_rate.toFixed(1)}% · явная рекомендация ${measured.recommendation_rate.toFixed(1)}% · предложен как вариант: ${measured.option_count ?? 0} ответов` : "Ещё не проверен: создайте новое исследование с YandexGPT"}</small>{measured?.competing_brands.length ? <p>Конкуренты: {measured.competing_brands.join(", ")}</p> : null}{measured?.citation_domains.length ? <p>Источники: {measured.citation_domains.join(", ")}</p> : null}</div><strong>{query.frequency.toLocaleString("ru-RU")}</strong></div>; })}<p className="method-note">Взвешенная видимость = сумма(частотность × доля рекомендаций бренда) / сумма частотностей только проверенных запросов. Непроверенные запросы не превращаются в нули.</p>{wordstatSnapshot.limitations.map((item) => <p className="method-note" key={item}>{item}</p>)}</div> : <div className="geo-empty"><strong>{loadErrors.Wordstat ? "Не удалось загрузить Wordstat" : loading ? "Загружаем Wordstat…" : "Снимка спроса пока нет"}</strong><p>{loadErrors.Wordstat ? loadErrors.Wordstat : loading ? "Проверяем доступ к данным спроса." : wordstatConnection?.connected ? "Введите бренд и категорию слева, выберите регион и нажмите «Найти запросы»." : "Подключите Wordstat в «Настройки → Интеграции», затем вернитесь сюда."}</p></div>}
      </section>
      <section className="analytics-card geo-site-audit yandex-intelligence">
        <div className="geo-audit-intro"><span className="eyebrow">ДОПОЛНИТЕЛЬНО · ДЛЯ ВЛАДЕЛЬЦА САЙТА</span><h2>Позиции и страницы из Яндекс Вебмастера</h2><p>Этот блок необязателен. Он добавляет данные подтверждённого сайта: показы, позиции, страницы и технические сигналы. Для Wordstat и проверки конкурентов право на сайт не требуется.</p><Button onClick={() => void syncYandex()} disabled={busy}>{busy ? "Синхронизируем…" : yandexIntelligence ? "Обновить данные владельца" : "Подключить Вебмастер"}</Button></div>
      ...
      <section className="analytics-card geo-site-audit alice-learning">
        <div className="geo-audit-intro"><span className="eyebrow">ШАГ 2 · ЗАКОНОМЕРНОСТИ YANDEXGPT API</span><h2>Что чаще встречается рядом с рекомендацией бренда</h2><p>Система сравнивает сохранённые ответы YandexGPT API, где бренд рекомендован или не рекомендован. Она проверяет наличие подходящей страницы, независимых источников, поисковую позицию и другие измеримые признаки. Пользовательская Алиса в эти данные не входит.</p><div className="geo-user-value"><b>Ценность для вас</b><p>Не общий совет «улучшайте контент», а проверяемая гипотеза: какой сигнал усилить и каким повторным исследованием YandexGPT API измерить результат.</p></div><Button onClick={() => void rebuildAlice()} disabled={busy}>{busy ? "Пересчитываем…" : "Обновить выводы по YandexGPT API"}</Button></div>
        {aliceLearning ? <div className="geo-audit-result"><header><div><strong>{aliceLearning.observation_count}</strong><span>наблюдений · {aliceLearning.recommendation_count} рекомендаций</span></div><Badge tone={aliceLearning.status === "READY" ? "success" : "warning"}>{aliceStatusLabel}</Badge></header>
          ...
          {aliceLearning.limitations.length ? <details><summary>Что важно учитывать</summary>{aliceLearning.limitations.map((item) => <p className="method-note" key={item}>{item}</p>)}</details> : null}</div> : <div className="geo-empty"><strong>{loadErrors["закономерности YandexGPT API"] ? "Не удалось загрузить историю" : loading ? "Загружаем историю…" : "Пока нет истории для сравнения"}</strong><p>Завершите первое исследование через YandexGPT API. Каждый сохранённый ответ станет наблюдением для будущего сравнения.</p></div>}
      </section>
      ...
      </section> : <section className="geo-platform-list">{platforms.filter((item) => item.source !== "YANDEX_SEARCH_GENERATIVE").map((item) => <article className="analytics-card record-card" key={item.id}><div><small>{item.category} · {item.country}/{item.language}</small><h2>{item.name}</h2><p>{item.domain} · Domain Trust: {item.domain_trust ?? "нет данных"} · Тематический авторитет: {item.topical_authority_score ?? "нет данных"}</p></div><Badge tone={item.active ? "success" : "neutral"}>{item.active ? "В РЕЕСТРЕ" : "ОТКЛЮЧЕНА"}</Badge></article>)}</section>}
      {publicationCandidates.length ? <section className="geo-platform-list"><div className="geo-ranking-head compact"><div><span className="eyebrow">ПЛАНЫ ПУБЛИКАЦИЙ</span><h2>Кандидаты из генеративного поиска Яндекса</h2><p>Список сформирован автоматически по источникам, использованным Яндексом в этом исследовании. Сначала проверьте редакционные правила и тематическое соответствие, затем сохраните публикацию и повторите тот же набор вопросов.</p></div></div>{publicationCandidates.map((item) => { const status = typeof item.evidence?.status === "string" ? item.evidence.status : ""; const topic = typeof item.evidence?.suggested_topic === "string" ? item.evidence.suggested_topic : "Проверить формат материалов площадки по теме исследования."; const observations = Array.isArray(item.evidence?.source_observations) ? item.evidence.source_observations.filter((entry): entry is { query?: string; url?: string; title?: string } => Boolean(entry && typeof entry === "object")) : []; return <article className="analytics-card record-card" key={`candidate-${item.id}`}><div><small>Наблюдение · генеративный поиск Яндекса</small><h2>{item.name}</h2><p>{item.domain}</p><p><b>Тема для проверки:</b> {topic}</p>{observations.length ? <details><summary>Доказательство из ответа Яндекса</summary>{observations.map((entry, index) => entry.url ? <p key={`${entry.url}-${index}`}><a href={entry.url} target="_blank" rel="noreferrer">{entry.title || entry.url}</a>{entry.query ? <><br />Запрос: «{entry.query}»</> : null}</p> : null)}</details> : <p className="method-note">Для старого кандидата доказательство доступно в исходном исследовании выше.</p>}<p className="method-note">Статус: <b>{status === "REVIEWED" ? "кандидат проверен" : status === "PLANNED" ? "публикация запланирована" : status === "PUBLISHED" ? "материал опубликован" : status === "READY_FOR_FOLLOWUP" ? "готов к повторному замеру" : "замечен в ответах"}</b>. Показатели EIS не рассчитаны, пока нет измеряемых входных данных.</p><PublicationCandidateTask key={item.id} item={item} busy={busy} onSave={(task) => savePublicationTask(item, task)} /></div><Badge tone="warning">КАНДИДАТ</Badge></article>; })}</section> : null}
    </main>
  );
}
  ...
  const downloadPlan = () => {
    ...
    const externalLink = (value: string, label?: string) => {
      try {
        ...
        if (!["https:", "http:"].includes(url.protocol) || url.username || url.password) return escapeHtml(label ?? value);
        return `<a href="${escapeHtml(url.href)}" target="_blank" rel="noopener noreferrer">${escapeHtml(label ?? value)}</a>`;
      } catch {
        return escapeHtml(label ?? value);
      }
    };
    const actions = (reportData.geo_opportunities ?? []).slice(0, 10).map((item, index) => {
      const evidence = (item.evidence ?? []).map((row) => `<li><b>${escapeHtml(row.query)}</b> · ${escapeHtml(row.provider)}/${escapeHtml(row.model)} · ответ #${escapeHtml(row.response_id)}${row.answer ? `<p><b>Сохранённый ответ:</b> ${escapeHtml(row.answer)}</p>` : ""}${row.competitors?.length ? `<p>Названы компании: ${row.competitors.map(escapeHtml).join(", ")}. Это не автоматически рекомендация.</p>` : ""}${row.urls.length ? `<ul>${row.urls.map((url) => `<li>${externalLink(url)}</li>`).join("")}</ul>` : "<br>Проверяемая ссылка отсутствует"}</li>`).join("");
      return `<article><h2>${index + 1}. ${escapeHtml(item.resource)}</h2><p><b>Почему:</b> ${escapeHtml(item.reason)}</p><p><b>Что сделать:</b> ${escapeHtml(item.deliverable)}</p><p><b>Ответственный:</b> ${escapeHtml(item.owner ?? "не назначен")}</p><p><b>Перед началом:</b> ${escapeHtml(item.prerequisites ?? "Проверить исходные данные и правила площадки.")}</p><p><b>Как проверить:</b> ${escapeHtml(item.verification)}</p><p><b>Ожидаемый эффект:</b> ${escapeHtml(item.effect_explanation ?? "Не установлен; требуется повторное измерение.")}</p><p><b>Ориентир по времени:</b> ${Number.isFinite(item.estimated_days) && item.estimated_days > 0 ? `${item.estimated_days} дней` : "не установлен"}</p>${evidence ? `<h3>Доказательства</h3><ul>${evidence}</ul>` : "<p>Доказательства для площадки пока не собраны: это задача по сбору данных.</p>"}<p><small>${escapeHtml(item.causality_notice)}</small></p></article>`;
    }).join("");
    const sources = (reportData.sources ?? []).filter((item) => item.url).map((item) => `<li>${externalLink(item.url!, item.title || item.source || item.url)}</li>`).join("");
    const sourceAnalysis = reportData.source_analysis ? `<h1>Разбор источников</h1><p>${escapeHtml(reportData.source_analysis.method)}</p><p>Успешных ответов: ${reportData.source_analysis.successful_responses}; исключено: ${reportData.source_analysis.excluded_responses}.</p>${reportData.source_analysis.resources.map((item) => `<article><h2>${escapeHtml(item.resource)}</h2><p>${escapeHtml(item.provider)}/${escapeHtml(item.model)} · упоминание со ссылкой: ${item.mentioned_with}/${item.with_source}; без ссылки: ${item.mentioned_without}/${item.without_source}</p><p>${escapeHtml(item.status)}</p><ul>${item.urls.map((url) => `<li>${externalLink(url)}</li>`).join("")}</ul><p>Вопросы: ${item.queries.map(escapeHtml).join("; ") || "не указаны"}</p></article>`).join("")}<p>${escapeHtml(reportData.source_analysis.limitation)}</p><p>${escapeHtml(reportData.source_analysis.next_step)}</p>` : "";
    const promptAppendix = reportData.query_catalog?.length ? `<h1>Приложение: зафиксированные вопросы</h1><ol>${reportData.query_catalog.map((item) => `<li>${escapeHtml(item.text)}${item.cluster ? ` <small>(${escapeHtml(item.cluster)})</small>` : ""}</li>`).join("")}</ol>` : "";
    const observations = reportData.yandex_generative_evidence?.observations ?? [];
    ...
    const limitations = (reportData.yandex_generative_evidence?.limitations ?? []).map(escapeHtml).join(" ");
    const html = `<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>GEO-план — ${escapeHtml(report.research.title)}</title><style>body{font:16px Arial,sans-serif;color:#162033;max-width:920px;margin:40px auto;padding:0 24px;line-height:1.5}header{border-bottom:3px solid #347cf7;margin-bottom:28px}h1{font-size:34px}article{padding:18px 0;border-bottom:1px solid #d8e0ea}small{color:#5b6575}a{color:#145ac5}@media print{body{margin:0}}</style></head><body><header><p>SIGNAL · исследование #${report.research.id}</p><h1>${escapeHtml(report.research.title)}</h1><p><b>${escapeHtml(scoreSummary)}</b></p><p>Сформировано: ${escapeHtml(new Date().toISOString())} · версия расчёта: ${escapeHtml(String(reportData.score?.version ?? "не указана"))}</p></header><h1>План действий</h1>${actions || "<p>Действия ещё не рассчитаны.</p>"}<h1>Источники со ссылками</h1><ul>${sources || "<li>В ответах текущего исследования ссылки не обнаружены.</li>"}</ul>${sourceAnalysis}${promptAppendix}${observationAppendix}<h1>Ограничения</h1><p>${limitations || "Отчёт фиксирует только исследованную выборку запросов, региона, времени и модели."}</p><p><small>Причинный эффект публикации подтверждается только повторным исследованием с тем же Frozen Prompt Set и контрольными вопросами. Отсутствующие данные не заменяются нулём.</small></p></body></html>`;
...
function Wizard({
  ...
  async function next() {
    ...
    if (step !== 2) return;
    if (!category.trim()) {
      setError("Укажите категорию спроса обычными словами, например «GEO-продвижение» или «онлайн-образование». Название бренда не заменяет категорию Wordstat.");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (status.connected) {
          const snapshot = await api.discoverWordstat({ brand, category: category.trim(), region_ids: region === "RU" ? [] : region === "RU-MILLION" ? millionIds : cityIds[region] ? [cityIds[region]] : [], device: "all", limit: queryLimit });
...
function Report({
  ...
  const discoveryResponses = report.explainability?.sample_scope?.successful_unbranded_response_count ?? successfulResponses;
  const brandedControls = report.explainability?.sample_scope?.excluded_branded_response_ids?.length ?? 0;
  const recommendedResponses = report.explainability?.recommendation_measurement?.recommended_responses ?? 0;
  const optionResponses = report.explainability?.option_measurement?.option_responses ?? 0;
    ...
    <main className="page report-page">
      ...
      <section id="actions" className="plan-section">
        ...
        {hasYandexGenerative ? <section className="panel research-lab-section">
          ...
          <p><b>Явные рекомендации: {primaryVisibility.toFixed(1)}% ({yandexGenerative?.recommendation_count ?? 0} из {yandexGenerative?.queries_measured ?? 0} ответов)</b> · предложен как вариант: {yandexGenerative?.option_count ?? 0} · упоминаний {yandexGenerative?.mention_count ?? 0} · ссылок на сайт {yandexGenerative?.target_citation_count ?? 0}. Взвешенный индекс этой выборки: {Number(yandexGenerative?.visibility_score ?? 0).toFixed(1)} из 100.</p>
