...
function Dashboard({
  ...
  const downloadPlan = () => {
    ...
    const actions = (reportData.geo_opportunities ?? []).slice(0, 10).map((item, index) => {
      const evidence = (item.evidence ?? []).map((row) => `<li><b>${escapeHtml(row.query)}</b> · ${escapeHtml(row.provider)}/${escapeHtml(row.model)} · ответ #${row.response_id}${row.urls.length ? `<ul>${row.urls.map((url) => `<li><a href="${escapeHtml(url)}">${escapeHtml(url)}</a></li>`).join("")}</ul>` : "<br>Проверяемая ссылка отсутствует"}</li>`).join("");
      const confidence = Number.isFinite(item.confidence) ? `${Math.round(item.confidence * 100)}% (эвристическая уверенность)` : "не установлена";
      return `<article><h2>${index + 1}. ${escapeHtml(item.resource)}</h2><p><b>Почему:</b> ${escapeHtml(item.reason)}</p><p><b>Что сделать:</b> ${escapeHtml(item.deliverable)}</p><p><b>Ответственный:</b> ${escapeHtml(item.owner ?? "не назначен")}</p><p><b>Перед началом:</b> ${escapeHtml(item.prerequisites ?? "Проверить исходные данные и правила площадки.")}</p><p><b>Как проверить:</b> ${escapeHtml(item.verification)}</p><p><b>Уверенность:</b> ${confidence} · срок ${item.estimated_days} дней</p>${evidence ? `<h3>Доказательства</h3><ul>${evidence}</ul>` : "<p>Доказательства для площадки пока не собраны: это задача по сбору данных.</p>"}<p><small>${escapeHtml(item.causality_notice)}</small></p></article>`;
    }).join("");
