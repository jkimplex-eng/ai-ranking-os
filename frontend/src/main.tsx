...
function CompetitorSocialPanel({ projectId, competitorId }: { projectId: number; competitorId: number }) {
  ...
  const saveTelegramProxy = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { setTelegram(await api.telegramSetProxy({ host: proxyHost.trim(), port: Number(proxyPort), username: proxyUsername.trim() || undefined, password: proxyPassword || undefined })); setProxyHost(""); setProxyUsername(""); setProxyPassword(""); } catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось подключить SOCKS5-прокси"); } finally { setBusy(false); } };
  ...
  return <details className="competitor-social"><summary>Соцсети и ежедневные публикации · {dashboard?.total_posts ?? 0}</summary>
    <section className="telegram-connection"><div className="telegram-connection__header"><div><span className="eyebrow">ПОИСК В ПУБЛИКАЦИЯХ</span><h3>Telegram</h3><p>Подключите служебный аккаунт один раз. API Hash, номер, сессия и 2FA не отображаются и не записываются в логи.</p></div><Badge tone={telegram?.configured ? "success" : telegram?.status === "PENDING_CODE" ? "warning" : "neutral"}>{telegram?.configured ? "ПОДКЛЮЧЕНО" : telegram?.status === "PENDING_CODE" ? "ОЖИДАЕТ КОД" : "НЕ ПОДКЛЮЧЕНО"}</Badge></div>
      ...
      {telegram?.configured ? <form className="telegram-proxy-form" onSubmit={saveTelegramProxy}><div className="telegram-proxy-form__title"><b>SOCKS5 для Telegram</b><Badge tone={telegram.proxy_configured ? "success" : "warning"}>{telegram.proxy_configured ? "АКТИВЕН" : "НЕ НАСТРОЕН"}</Badge></div><input aria-label="Адрес SOCKS5" placeholder="Адрес или IP прокси" value={proxyHost} onChange={(event) => setProxyHost(event.target.value)} required /><input inputMode="numeric" aria-label="Порт SOCKS5" placeholder="Порт" value={proxyPort} onChange={(event) => setProxyPort(event.target.value.replace(/\D/g, ""))} required /><input aria-label="Логин SOCKS5" placeholder="Логин — если требуется" value={proxyUsername} onChange={(event) => setProxyUsername(event.target.value)} /><input type="password" autoComplete="off" aria-label="Пароль SOCKS5" placeholder="Пароль — если требуется" value={proxyPassword} onChange={(event) => setProxyPassword(event.target.value)} /><button className="secondary" disabled={busy}>{busy ? "Проверяем…" : "Проверить и подключить"}</button>{telegram.proxy_configured ? <button type="button" className="ghost" onClick={clearTelegramProxy} disabled={busy}>Отключить прокси</button> : null}<small>Прокси проверяется соединением с Telegram и сохраняется только после успешной проверки. Пароль хранится зашифрованным.</small></form> : null}
