...
function GeoOpportunitiesScreen() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      api.finalReport(selectedResearchId).then((report) => { if (!cancelled) setResearchEvidence(report as ReportShape); }).catch((reason) => { if (!cancelled) setEvidenceError(reason instanceof Error ? reason.message : "Ошибка загрузки отчёта"); });
    }, 0);
  ...
  const publicationCandidates = platforms.filter((item) => item.source === "YANDEX_SEARCH_GENERATIVE");
  return (
