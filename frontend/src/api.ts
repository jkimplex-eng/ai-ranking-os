...
export type GeoAuditCheck = { code: string; category: string; title: string; passed: boolean; points: number; max_points: number; evidence: string; recommendation?: string; checked_url?: string };
export type GeoSiteAudit = { id: number; project_id?: number; brand: string; website_url: string; final_url: string; score: number; grade: string; category_scores: Record<string, number>; checks: GeoAuditCheck[]; opportunities: Array<{ priority: string; problem: string; affected_metric: string; action: string; expected_effect: string; confidence: string; effort: string; verification: string; checked_url?: string }>; evidence: Record<string, unknown>; algorithm_version: string; limitation: string; created_at: string };
...
export type WordstatConnection = { connected: boolean; status: string; folder_id?: string; auth_type?: "API_KEY" | "IAM_TOKEN"; last_checked_at?: string; last_success_at?: string; last_error?: string; managed_by_platform?: boolean };
...
export type WordstatAnalyticsItem = { query: string; frequency: number; demand_rank: number; response_count: number; mention_count: number; recommendation_count: number; option_count?: number; mention_rate: number; recommendation_rate: number; competing_brands: string[]; citation_domains: string[]; evidence_status: string; research_ids: number[] };
...
export class ApiClient {
  ...
  deleteCompetitorSocialPost(projectId: number, competitorId: number, sourceId: number, postId: number) { return this.request<void>(`/competitor-intelligence/projects/${projectId}/competitors/${competitorId}/social/${sourceId}/posts/${postId}`, { method: "DELETE" }); }
  ...
  telegramSetProxy(payload: { protocol: "SOCKS5" | "HTTP" | "MTPROXY"; host: string; port: number; username?: string; password?: string; secret?: string }) { return this.request<TelegramConnection>("/competitor-intelligence/telegram/connection/proxy", { method: "PUT", body: JSON.stringify(payload) }); }
